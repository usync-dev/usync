...
import { GoogleAuthorizer, type TokenData } from "@usync/oauth2";
import assert from "node:assert";
import * as fs from "node:fs";
import * as path from "node:path";
import * as readline from "node:readline";

const CLIENT_ID = process.env.GOOGLE_DESKTOP_ID!;
const CLIENT_SECRET = process.env.GOOGLE_DESKTOP_SECRET;
const REDIRECT_URL = "http://localhost:5678/callback";

const INITIAL_CONTENT = "hello from googledrive demo";
...
async function main() {
  const tokenPath = path.resolve(".tokens", "googledrive.json");
  ...
  if (!tokens.refreshToken?.token) {
    ...
    const authorizer = new GoogleAuthorizer({
      clientId: CLIENT_ID,
  ...
  console.log("\n✓ Google Drive demo passed");
}

main().catch((err) => {
  console.error("✗ Google Drive demo failed:", err);
  process.exit(1);
});
